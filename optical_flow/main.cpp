...
int main(int argc, char** argv)
{
  ...
  while(1){
    ...
    Mat colorA,imgA;
    cap.read(colorA); // read a new frame from video
    cvtColor(colorA,imgA, CV_BGR2GRAY);
    usleep(1000*1000);
    Mat colorB,imgB;
    cap.read(colorB); // read a new frame from video
    cvtColor(colorB,imgB, CV_BGR2GRAY);
